...
function renderGroupedByLeague(builtRows) {
...
function renderHistoryPage(type) {
  ...
  if (dates.length === 0) {
    targetEl.innerHTML = '<div class="empty-state"><div class="e-icon">📋</div><h3>No history yet</h3><p>Past results will appear here</p></div>';
    return;
  }
  ...
  const leaguesHTML = matches.map(m => {
    const league = buildLeagueLabel(m) || "OTHER MATCHES";
    const status = getStatus(m);
    return `
    <div class="league-group">
      ${buildLeagueHeader(league, m.time || "--:--", status)}
